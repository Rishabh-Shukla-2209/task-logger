...
import { FollowupEventForm } from "./FollowupEventForm"

export default async function AdminFollowupDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" render={<Link href="/admin/followups" />}>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div>
          {followup.status === "ACTIVE" && (
            <FollowupEventForm followupId={followup.id} />
          )}
        </div>
      </div>
    </div>
