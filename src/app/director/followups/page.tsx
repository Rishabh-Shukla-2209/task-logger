...
export default async function DirectorFollowupsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const currentStatus = searchParamsAwaited.status === "RESOLVED" ? "RESOLVED" : "ACTIVE"
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">View and monitor payment followups.</p>
      </div>
                  ...
                  followups.map((f) => (
                    <TableRow key={f.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" render={<Link href={`/director/followups/${f.id}`} />}>
